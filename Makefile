...
argocd-install:
	...
	kubectl apply --namespace $(ARGOCD_NAMESPACE) --filename ./.dev_config/argocd-server-nodeport.yaml
...
crossplane-install: crossplane-configure-repo
	kubectl apply --filename .dev_config/crossplane-app.yaml
	@sleep 60s
	kubectl apply --filename .dev_config/controllerconfig.yaml

# Clean up Crossplane and ArgoCD installations
crossplane-clean:
	argocd repo rm $(CROSSPLANE_REPO_URL)  
	kubectl delete --filename .dev_config/controllerconfig.yaml 
	kubectl delete --filename .dev_config/crossplane-app.yaml
